...
/**
 * This module act as a gateway between adyogi dl and external systems like google GTM window.dataLayer
 * If needed be this script can also send data to any backend service
 */

const helper = __webpack_require__(/*! ./helper.js */ "./src/handlers/helper.js");
...
// the properties in plain text is hashed before sending
 ...
 *          id: the unique id of event
 *          timestamp: the timestamp associated with this event
 *      }
 *      device: {
...
// This is the copy of event names in pixel.js file
// pixel.js file is uploaded to shopify which loads the listener script
// so we need to resolve pixel event names in listener script
const PIXEL_EVENT_NAME_COPY = {
    PAGE_VIEW: 'PAGE_VIEW',
    ...
    SHIPPING_INFO_SUBMITTED: 'SHIPPING_INFO_SUBMITTED',
...
// there may be a workaround, whenever we get contact info submitted event
...
const collectCustomerDetails = function (name, event) {
    ...
    if (
        name === PIXEL_EVENT_NAME_COPY.BEGIN_CHECKOUT ||
        name === PIXEL_EVENT_NAME_COPY.PAYMENT_INFO_SUBMITTED ||
        name === PIXEL_EVENT_NAME_COPY.SHIPPING_INFO_SUBMITTED ||
        name === PIXEL_EVENT_NAME_COPY.PURCHASE
    ) {
...
const lineItemsToItems = function (lineItems) {
    ...
    for (const li of lineItems) {
        ...
        if (_.has(li, 'merchandise.price')) {
            i.price = li.merchandise.price.amount;
        }
        items.push(i);
    }
    return items;
...
// begin checkout --> currency, value, items
// add payment info --> currency, value, items, payment type
// add shipping info --> currency, value, items, shipping tier
// purchase --> currency, value, items, tax, transaction id
// remove from cart --> currency, value, items
// view cart --> currency, value, items
// view item --> currency, value, items
// view collection --> currency, item list name, item list id, items
// page view --> url, pathname
const collectEcommerceDetails = function (name, event) {
    let currency = null,
        value = null,
        items = null,
        tax = null,
    ...
    if (name === PIXEL_EVENT_NAME_COPY.PRODUCT_VIEW) {
    ...
    if (name === PIXEL_EVENT_NAME_COPY.COLLECTION_VIEW) {
        if (_.has(event, 'data.collection')) {
            ili = event.data.collection.id;
            iln = event.data.collection.title;
        }
...
// this data is preserved between browser reloads
const storeData = function (key, data) {
    localStorage.setItem(key, data);
};

// fetch the data stored against the key or if no data is present return undefined
const fetchData = function (key) {
    return localStorage.getItem(key);
};

// Prepare event schema, the name mapping is given above
// the event object is provided by shopify
// see shopify web pixel api for more details
// https://shopify.dev/docs/api/web-pixels-api/standard-events
const prepareEventSchema = async function (name, event) {
    const customer = collectCustomerDetails(name, event);
    const device = collectDeviceDetails(name, event);
    const marketing = collectMarketingDetails(name, event);
...
const helper = __webpack_require__(/*! ./helper */ "./src/handlers/helper.js");
const gateway = __webpack_require__(/*! ./gateway */ "./src/handlers/gateway.js");

// The maximum time ip address is cached
const IP_TIME_DELTA_MS = 30 * 60 * 1000;
...
(function () {
    ...
    const init = async function () {
        ...
        if (!exists && window) {
            ...
            helper.storeData(helper.StoreKeys.IP, JSON.stringify({ ip: ipResponse.ip, timestamp: Date.now() }));
        }
    };
...
(function (w, d) {
    ...
    if (w.agdl) {
        if (typeof w.agdl.handlerMethod !== 'function') {
            w.agdl.handlerMethod = async function () {
                await handlers.handle(arguments[0], arguments[1]);
            };
        }
    }
